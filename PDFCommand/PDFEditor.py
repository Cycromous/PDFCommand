...
class EditorFrame(tk.Frame):
    ...
    def create_tool_button(self, parent, text, bg_color, fg_color, command, width=100, height=36):
        ...
        return create_rounded_button(parent, text, bg_color, fg_color, command,
                                      width=width, height=height, canvas_bg=TOOLBAR_COLOR,
    ...
    def setup_gui(self):
        ...
        tk.Button(inner_toolbar, text="Home", command=self.go_home, bg=TOOLBAR_COLOR,
                  fg=TEXT_COLOR, font=("Segoe UI", 10, "bold"), bd=0,
                  activebackground=TOOLBAR_COLOR, cursor="hand2").pack(side=tk.LEFT, padx=(0, 15))

        tk.Frame(inner_toolbar, bg="#71C89F", width=2).pack(side=tk.LEFT, fill=tk.Y, pady=5, padx=10)
        btn_open = create_rounded_button(inner_toolbar, "Open PDF", "#FFFFFF", "#374151",
                                          self.open_pdf, width=140, canvas_bg=TOOLBAR_COLOR)
        btn_open[0].pack(side=tk.LEFT, padx=(0, 10))
        ...
        tools_frame = tk.Frame(inner_toolbar, bg=TOOLBAR_COLOR)
        ...
        tk.Label(tools_frame, text="Active Tool", bg=TOOLBAR_COLOR, fg=TEXT_COLOR,
                 font=("Segoe UI", 9, "bold")).pack(side=tk.TOP, anchor="w")

        btns_frame = tk.Frame(tools_frame, bg=TOOLBAR_COLOR)
        btns_frame.pack(side=tk.TOP)
        
        self.btn_text = self.create_tool_button(btns_frame, "Text", "#111827", "#FFFFFF", lambda: self.set_mode("text"))

        self.btn_text = self.create_tool_button(btns_frame, "Text", "#111827", "#FFFFFF",
        ...
        self.btn_shape = self.create_tool_button(btns_frame, "Shape", "#FFFFFF", "#374151",
        ...
        self.shape_combo = ttk.Combobox(btns_frame, textvariable=self.current_shape_type,
                                         values=("rectangle", "oval"), width=8, state="readonly")
        ...
        self.btn_color_canvas = tk.Canvas(btns_frame, width=32, height=32, bg=TOOLBAR_COLOR,
                                           highlightthickness=0, cursor="hand2")
        self.color_circle = self.btn_color_canvas.create_oval(4, 4, 28, 28, fill=self.shape_color,
        ...
        font_size_frame = tk.Frame(inner_toolbar, bg=TOOLBAR_COLOR)
        ...
        tk.Label(font_size_frame, text="Font Size", bg=TOOLBAR_COLOR, fg=TEXT_COLOR,
                 font=("Segoe UI", 9, "bold")).pack(side=tk.TOP, anchor="w")

        slider_canvas = tk.Canvas(font_size_frame, width=180, height=50, bg=self.toolbar_color, highlightthickness=0, cursor="hand2")
        slider_canvas = tk.Canvas(font_size_frame, width=180, height=50, bg=TOOLBAR_COLOR,
                                   highlightthickness=0, cursor="hand2")
        ...
        def _draw_slider(val):
            ...
            slider_canvas.create_oval(cx - 14, 25 - 14, cx + 14, 25 + 14, fill="#2ECC8A",
        ...
        btn_save = create_rounded_button(inner_toolbar, "Save As", "#111827", "#FFFFFF",
                                          self.save_as_pdf, width=120, canvas_bg=TOOLBAR_COLOR)
        btn_save[0].pack(side=tk.RIGHT, padx=5)

        self.lbl_save_status = tk.Label(inner_toolbar, text="", bg=TOOLBAR_COLOR, fg="#059669",
                                         font=("Segoe UI", 9, "bold"))
        ...
        self.lbl_instructions = tk.Label(inner_toolbar, text="Drag: Move | Click Image: Show Resize Corners",
                                          bg=TOOLBAR_COLOR, fg=TEXT_COLOR, font=("Segoe UI", 10))
        ...
        self.btn_prev = tk.Button(self.nav_frame, text="Previous Page", command=self.prev_page,
                                   font=("Segoe UI", 9, "bold"), bg="#FFFFFF", bd=0, padx=15, pady=4,
                                   cursor="hand2")
        ...
        self.lbl_page_info = tk.Label(self.nav_frame, text="Page 0 of 0", font=("Segoe UI", 10, "bold"),
                                       bg=NAV_BG, fg="#374151")
        ...
        self.btn_next = tk.Button(self.nav_frame, text="Next Page", command=self.next_page,
                                   font=("Segoe UI", 9, "bold"), bg="#FFFFFF", bd=0, padx=15, pady=4,
                                   cursor="hand2")
        self.btn_next.pack(side=tk.RIGHT, padx=20)

        canvas_frame = tk.Frame(self.root, bg="#F0F2F5", bd=0)
        canvas_frame = tk.Frame(self, bg=BG_GRAY, bd=0)
    ...
    def render_page(self):
        ...
        for item_id, data in list(self.master_shape_items[self.current_page_num].items()):
            ...
            shape_type = data.get("type", "rectangle")
            if shape_type == "rectangle":
                new_id = self.canvas.create_rectangle(cx1, cy1, cx2, cy2, fill=data["color"],
                                                        outline=data["color"], tags=("draggable", "shape"))
            else:
                new_id = self.canvas.create_oval(cx1, cy1, cx2, cy2, fill=data["color"],
                                                   outline=data["color"], tags=("draggable", "shape"))

            self.master_shape_items[self.current_page_num][new_id] = \
                self.master_shape_items[self.current_page_num].pop(item_id)
            self.master_shape_items[self.current_page_num][new_id]["true_coords"] = (tx1, ty1, tx2, ty2)
        ...
        for item_id, data in list(self.master_text_items[self.current_page_num].items()):
            ...
            new_id = self.canvas.create_text(cx, cy, text=data["text"],
                                              font=("Times New Roman", -max(1, scaled_size)),
                                              anchor=tk.SW, fill="#000000", tags=("draggable",))
            self.master_text_items[self.current_page_num][new_id] = \
                self.master_text_items[self.current_page_num].pop(item_id)
            self.master_text_items[self.current_page_num][new_id]["true_coords"] = (true_x, true_y)
        ...
        for item_id, data in list(self.master_image_items[self.current_page_num].items()):
            ...
            new_id = self.canvas.create_image(cx, cy, anchor=tk.NW, image=tk_img, tags=("draggable", "image"))
            self.tk_images[new_id] = tk_img

            self.master_image_items[self.current_page_num][new_id] = \
                self.master_image_items[self.current_page_num].pop(item_id)
            self.master_image_items[self.current_page_num][new_id]["true_coords"] = (true_x, true_y)
    ...
    def update_handles(self):
        ...
        r = 5
        self.canvas.create_rectangle(x - r, y - r, x + r, y + r, fill="#FFFFFF", outline="#2ECC8A", width=2,
                                      tags=("handle", "nw"))
        self.canvas.create_rectangle(x + w - r, y - r, x + w + r, y + r, fill="#FFFFFF", outline="#2ECC8A",
                                      width=2, tags=("handle", "ne"))
        self.canvas.create_rectangle(x - r, y + h - r, x + r, y + h + r, fill="#FFFFFF", outline="#2ECC8A",
                                      width=2, tags=("handle", "sw"))
        self.canvas.create_rectangle(x + w - r, y + h - r, x + w + r, y + h + r, fill="#FFFFFF",
                                      outline="#2ECC8A", width=2, tags=("handle", "se"))
    ...
    def add_image(self):
        ...
        filepath = filedialog.askopenfilename(title="Select Image or Signature",
                                               filetypes=[("Image Files", "*.png;*.jpg;*.jpeg;*.bmp")])
        if not filepath:
            return
        ...
        try:
            ...
            item_id = self.canvas.create_image(spawn_x, spawn_y, anchor=tk.NW, image=tk_img,
                                                tags=("draggable", "image"))
            ...
            self.master_image_items[self.current_page_num][item_id] = {
                ...
                "width": img_page.rect.width,
                "height": img_page.rect.height,
                "true_coords": ((spawn_x - self.x_offset) / self.scale_factor,
                                (spawn_y - self.y_offset) / self.scale_factor),
            }
    ...
    def on_canvas_drag(self, event):
        if self.drag_data.get("handle"):
            ...
            new_scale = nw / orig_w
            new_scale = nw / orig_w if orig_w else 1.0
    ...
    def on_canvas_release(self, event):
        data_changed = False
        if self.drag_data.get("handle"):
            if self.selected_image and self.canvas.coords(self.selected_image):
                coords = self.canvas.coords(self.selected_image)
                true_x = (coords[0] - self.x_offset) / self.scale_factor
                true_y = (coords[1] - self.y_offset) / self.scale_factor
                self.master_image_items[self.current_page_num][self.selected_image]["true_coords"] = (true_x, true_y)
            self.update_handles()
    ...
    def on_right_click(self, event):
        ...
        if item and "draggable" in self.canvas.gettags(item[0]):
            ...
            if item_id in self.master_text_items[self.current_page_num]:
                del self.master_text_items[self.current_page_num][item_id]
            if item_id in self.master_shape_items[self.current_page_num]:
                del self.master_shape_items[self.current_page_num][item_id]
            if item_id in self.master_image_items[self.current_page_num]:
                del self.master_image_items[self.current_page_num][item_id]
            if item_id in self.tk_images:
                del self.tk_images[item_id]
    ...
    def add_text_prompt(self, x, y):
        ...
        if text:
            ...
            item_id = self.canvas.create_text(x, y, text=text,
                                               font=("Times New Roman", -max(1, scaled_size)),
                                               anchor=tk.SW, fill="#000000", tags=("draggable",))
    ...
    def trigger_autosave(self):
        ...
        if not self.current_filepath or not self.pdf_bytes:
            return
    ...
    def _perform_save_to_path(self, filepath):
        ...
        snapshot = {
            "text": {p: dict(items) for p, items in self.master_text_items.items()},
            "shape": {p: dict(items) for p, items in self.master_shape_items.items()},
            "image": {p: dict(items) for p, items in self.master_image_items.items()},
        }

        threading.Thread(target=self._perform_save_to_path,
                          args=(self.current_filepath, snapshot), daemon=True).start()

    def _perform_save_to_path(self, filepath, snapshot=None):
        try:
            if snapshot is None:
                snapshot = {
                    "text": {p: dict(items) for p, items in self.master_text_items.items()},
            ...
            for p_num in range(temp_doc.page_count):
                ...
                for data in snapshot["shape"].get(p_num, {}).values():
                    tx1, ty1, tx2, ty2 = data["true_coords"]
                ...
                for data in snapshot["image"].get(p_num, {}).values():
                    tx1, ty1 = data.get("true_coords", (50 / self.scale_factor, 50 / self.scale_factor))
                    tx2 = tx1 + (data["current_width"] / self.scale_factor)
                    ty2 = ty1 + (data["current_height"] / self.scale_factor)
                    rect = fitz.Rect(tx1, ty1, tx2, ty2)
                ...
                for data in snapshot["text"].get(p_num, {}).values():
                    true_x, true_y = data["true_coords"]
                    adjusted_y = true_y - (data["size"] * 0.20)
    ...
    def save_as_pdf(self):
        ...
        if filepath:
            ...
            if self._perform_save_to_path(filepath):
                messagebox.showinfo("Success", "PDF downloaded successfully!")
            else:
